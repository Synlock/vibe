...
class MyMicStream extends StatefulWidget {
  ...
  MyMicStream({Key? key, isRecording}) : super(key: key);
  @override
  State<MyMicStream> createState() => MyMicStreamState();
}
...
class MyMicStreamState extends State<MyMicStream>
  ...
  void controlMicStream({Command command = Command.change}) async {
    switch (command) {
      case Command.change:
        await changeListening(widget.isRecording);
        break;
      case Command.start:
  ...
  Future<void> initPlatformState() async {
    ...
    controller =
        AnimationController(duration: const Duration(seconds: 1), vsync: this)
          ..addListener(() {
            if (widget.isRecording) setState(() {});
          })
  ...
  Color _getBgColor() => (widget.isRecording) ? Colors.red : Colors.cyan;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      ...
      memRecordingState = widget.isRecording;
...
class StartMicStream extends StatefulWidget {
  const StartMicStream({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => StartMicStreamState();
}

class StartMicStreamState extends State<StartMicStream>
    with SingleTickerProviderStateMixin, WidgetsBindingObserver {
  Stream? stream;
  late StreamSubscription listener;
  List<int>? currentSamples = [];
